...
class SrdanetGenerator(nn.Module):
    def __init__(self, num_cls=2):
        ...
        SRAAN_up2 = [nn.PixelShuffle(2),
                            nn.ReLU(True)]
        SRAAN_up4 = [nn.PixelShuffle(2),
                            nn.ReLU(True)]
        SRAAN_classifier = [nn.Conv2d(16, 3, kernel_size=9, padding=4)]
        ...
        '''
        #SSAN_pixel
        self.SSAN_mix_conv1 = nn.Conv2d(320, 160, kernel_size=3, padding=1)
        self.SSAN_mix_conv2 = nn.Conv2d(176, 88, kernel_size=3, padding=1)
        self.SSAN_classifier = nn.Conv2d(88, num_cls, kernel_size=9, padding=4)
        #faloss
        self.pointwise1 = torch.nn.Sequential(
            torch.nn.Conv2d(256,3,1),
            torch.nn.BatchNorm2d(3),  #添加了BN层
            torch.nn.ReLU(inplace=True)
        )
        self.pointwise2 = torch.nn.Sequential(
    ...
    def forward(self, x):
        ...
        #超分辨
        SRAAN_up2 = self.SRAAN_up2(feature) 
        SRAAN_up4 = self.SRAAN_up4(SRAAN_up2) 
        sr_image = self.SRAAN_classifier(SRAAN_up4) 
        ...
        # return feature, pre_seg, sr_image  # self.feature_A, self.pre, self.fakeB
        # return feature, pre_seg, sr_image,self.pointwise1(SSAN_feature_up2),self.pointwise2(SRAAN_up2)
        return feature, pre_seg, sr_image,_,_
...
def ssmsrdanet_generator(num_cls=2, gpu_ids=[]):
...
class ssmSrdanetDs(nn.Module):

	def __init__(self, num_classes, ndf = 64):
		super(ssmSrdanetDs, self).__init__()
	...
	def forward(self, x):
		...
		return x
def ssmsrdanet_ds(num_classes=2, gpu_ids=[]):
    model = ssmSrdanetDs(num_classes)
    if len(gpu_ids) > 0:
        assert (torch.cuda.is_available())
        model.to(gpu_ids[0])
        model = torch.nn.DataParallel(model, gpu_ids)  # multi-GPUs
...
################################################
#SSM-SRDA baseline model
################################################
class ssmSrdanet_Baseline(nn.Module):
    def __init__(self, num_cls=2):
        super(ssmSrdanet_Baseline, self).__init__()
        self.feature_extractor = nn.Sequential(
            nn.Conv2d(3, 64, 3, 1, 1, bias=False),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(64, 128, 3, 1, 1, bias=False),
    ...
    def forward(self, x):
        ...
        return  pre
def ssmsrdanet_baseline(num_cls=2, gpu_ids=[]):
    model = ssmSrdanet_Baseline(num_cls)  # out [-1, 1]
    if len(gpu_ids) > 0:
        assert (torch.cuda.is_available())
        model.to(gpu_ids[0])
        model = torch.nn.DataParallel(model, gpu_ids)  # multi-GPUs
